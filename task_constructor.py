...
from ofa_datasets import (GraphListDataset, SubgraphDataset, MultiDataset, GraphListHierDataset, SubgraphHierDataset,
                          ...
                          GraphListNopromptDataset, SubgraphNopromptLinkDataset, )
...
def ConstructNodeCls(dataset, split, split_name, prompt_feats, to_bin_cls_func, global_data, **kwargs):
    ...
    return SubgraphHierDataset(text_g, prompt_feats["class_node_text_feat"], prompt_feats["prompt_edge_text_feat"],
                               ...
                               prompt_edge_list={"f2n": [1, None], "n2f": [3, None], "n2c": [2, None],
                                                 "c2n": [4, None]}, **kwargs, )
...
def get_lr_eval_data(self, construct, data, g, config, split, eval_construct_config, eval_args):
    eval_data = [construct(dataset=g if g else self.dataset[data], data_manager=self.datamanager.get(data), n=n, k=k,
                           split_name=eval_construct_config["stage"], config=config, split=split,
                           state_name=f'{eval_construct_config["stage"]}_fs{n}_{k}_{data}' if n else f'{eval_construct_config["stage"]}_fs{k}_{data}',
                           eval_metric=config["eval_metric"], eval_func=globals()[config["eval_func"]],
                           batch_size=self.batch_size, **eval_args, ) for n in eval_args["val_n"] for k in
                 eval_args["val_k"]]
    return eval_data
...
def get_e2e_eval_data(self, construct, data, task, split, global_data, config, eval_construct_config, eval_args):
    if "process_label_func" in eval_construct_config:
        trim_class_func = globals()[eval_construct_config["process_label_func"]]
    else:
        trim_class_func = globals()[config["process_label_func"]]

    eval_data = construct(task, self.dataset[data], split, eval_construct_config["split_name"], trim_class_func,
                          global_data=global_data, **eval_args, )
    dm_data = make_data(task, eval_data, eval_construct_config["split_name"], config["eval_metric"],
                        globals()[config["eval_func"]], config["num_classes"], batch_size=self.batch_size,
...
def get_graph(self, data, task_level, class_split_lst=None):
    ...
    if "link" in task_level:
        if data not in self.edges:
            ...
            converted_triplet = dataset.get_idx_split()
            edges = torch.cat(
                [torch.tensor(converted_triplet["train"][0]).T, torch.tensor(converted_triplet["valid"][0]).T,
                 torch.tensor(converted_triplet["test"][0]).T, ], dim=-1, )
            self.edges[data]["edges"] = edges
            self.edges[data]["fs_edges"] = [[], [], edges]
            edge_labels = torch.cat(
                [torch.tensor(converted_triplet["train"][1]), torch.tensor(converted_triplet["valid"][1]),
            ...
            if class_split_lst is not None:
                ...
                for classes in class_split_lst:
                    fs_mask = torch.tensor([item in classes for item in edge_labels])
